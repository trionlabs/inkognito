      ...
      <div className="space-y-6">
        {pdfFile && (
          <div className="border border-green-400/30 bg-green-400/5 p-3 flex items-center gap-3 success-glow">
            ...
            <span className="font-mono text-sm text-ink-text truncate flex-1">{pdfFile.name}</span>
            <span className="font-mono text-sm text-ink-muted">{formatBytes(pdfFile.size)}</span>
          </div>
        )}

        <p className="font-mono text-sm text-ink-subtle text-center">
        ...
        <div className="flex justify-center">
          {selfApp ? (
            ...
            <div className="w-[280px] h-[280px] border border-ink-border flex items-center justify-center">
              <p className="font-mono text-sm text-ink-muted">Loading QR Code...</p>
            </div>
          )}
        </div>
        ...
        {showFallback && !proofError && (
          <div className="flex justify-center animate-fadeInUp">
            <button
              onClick={onSuccess}
              className="font-mono text-sm text-ink-muted hover:text-ink-subtle border border-ink-border hover:border-ink-border-strong px-4 py-2 transition-all duration-200 uppercase tracking-[0.12em]"
            >
        ...
        {proofError && (
          <div className="space-y-3">
            <p className="font-mono text-sm text-red-400 text-center">{proofError}</p>
            <div className="flex gap-3 justify-center">
              <button
                onClick={() => setProofError(null)}
                className="border border-ink-border-strong hover:border-ink-subtle px-4 py-2 font-mono text-sm text-ink-subtle hover:text-ink-bright transition-all duration-200 uppercase tracking-[0.15em]"
              >
                Try again
              </button>
              <button
                onClick={onBack}
                className="border border-ink-border hover:border-ink-subtle px-4 py-2 font-mono text-sm text-ink-muted hover:text-ink-bright transition-all duration-200 uppercase tracking-[0.15em]"
              >
                Go back
              </button>
            </div>
