      ...
      <div className="space-y-6">
        {success && proveResult ? (
            ...
            {identity ? (
              <div className="border border-ink-border border-l-green-400/40 bg-ink-surface/60 animate-fadeInUp">
                ...
                <div className="px-6 py-4 border-b border-ink-border flex items-center gap-3">
                  ...
                  <span className="font-mono text-[12px] uppercase tracking-[0.25em] text-green-400">
                    Verified
                  </span>
                </div>
                ...
                <div className="px-6 py-5 grid grid-cols-2 gap-x-8 gap-y-5">
                  {identity.given_name && (
                    <div>
                      <p className="font-mono text-[12px] text-ink-muted uppercase tracking-[0.15em] mb-1.5">Name</p>
                  ...
                  {identity.nationality && (
                    <div>
                      <p className="font-mono text-[12px] text-ink-muted uppercase tracking-[0.15em] mb-1.5">Nationality</p>
                      <p className="font-mono text-base text-ink-bright">{identity.nationality}</p>
                    </div>
                  )}
                  {identity.dob && (
                    <div>
                      <p className="font-mono text-[12px] text-ink-muted uppercase tracking-[0.15em] mb-1.5">Date of Birth</p>
                      <p className="font-mono text-base text-ink-bright">{identity.dob}</p>
                    </div>
                  )}
                  {identity.older_than && (
                    <div>
                      <p className="font-mono text-[12px] text-ink-muted uppercase tracking-[0.15em] mb-1.5">Age Verified</p>
                ...
                {useCase && useCase.checks.length > 0 && (
                  <div className="px-6 py-4 border-t border-ink-border">
                    <p className="font-mono text-[12px] text-ink-muted uppercase tracking-[0.15em] mb-2.5">
                      ZK proof verifies
                    </p>
                    <div className="flex flex-wrap gap-2">
                      {useCase.checks.map((check) => (
                        <span
                          key={check}
                          className="font-mono text-[12px] text-green-400/80 border border-green-400/20 px-2.5 py-1"
                        >
                          {check}
                        </span>
                      ))}
            ...
            >
              <div className="px-6 py-4 flex items-center justify-between border-b border-ink-border">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="font-mono text-[13px] uppercase tracking-[0.2em] text-ink-subtle">
                    Succinct Prover Network
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <div className={`w-1.5 h-1.5 rounded-full ${statusDotColor(networkStatus)}`} />
                  <span className={`font-mono text-[13px] ${statusColor(networkStatus)}`}>
              ...
              <div className="px-6 py-4 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <span className="font-mono text-[12px] text-ink-muted uppercase tracking-[0.1em]">Request ID</span>
                  <div className="relative flex items-center gap-2">
                    <code className="font-mono text-[13px] text-ink-subtle">
                    ...
                    {copied && (
                      <span className="copy-tooltip absolute -top-6 right-0 font-mono text-[11px] text-ink-bright bg-ink-surface border border-ink-border-strong px-2 py-0.5">
                        Copied!
                      </span>
                    )}
                  </div>
                ...
                <a
                  ...
                  className="group w-full border border-ink-border-strong hover:border-ink-subtle py-3 font-mono text-[14px] text-ink-text hover:text-ink-bright transition-all duration-200 uppercase tracking-[0.15em] flex items-center justify-center gap-2"
                >
          ...
          <div className="border border-ink-border bg-ink-surface/40 p-8 text-center space-y-3">
            <p className="font-mono text-[15px] text-red-400">Proof generation failed</p>
            {proveError && (
              <p className="font-mono text-[13px] text-ink-muted">{proveError}</p>
            )}
          </div>
        )}
        ...
        <button
          onClick={onReset}
          className="w-full border border-ink-border-strong hover:border-ink-subtle py-3 font-mono text-[14px] text-ink-subtle hover:text-ink-bright transition-all duration-200 uppercase tracking-[0.15em] animate-fadeInUp"
